 ...
 *
 * Passaggio delle carte da un telefono all'altro con uno o più QR.
 *
 * Chi cede mostra i codici, chi riceve li inquadra. Niente rete, niente file,
 * niente account. Serve dentro la famiglia, quindi il contenuto non è cifrato:
 * chi legge lo schermo legge le tessere, ed è una scelta presa sapendola.
 *
 * Come è fatto un trasferimento, dall'interno verso l'esterno:
 *
 ...
 *   binario compresso ne costa 20: è la differenza fra due carte e quaranta
 *   dentro lo stesso QR.
 * - **base44 e non base64.** L'alfabeto di base44 sta tutto dentro la modalità
 *   alfanumerica del QR, che impacchetta due caratteri in 11 bit: costa il 3
 *   per cento di capacità invece del 25. Rispetto al base45 dello standard
 *   manca solo lo spazio, tolto apposta perché il QR di OpenCard passa per
 ...
 * ogni QR dice quale pezzo è e quanti sono in tutto, e porta il CRC32
...
/* Quanti QR al massimo. Con 950 byte a pezzo sono più di duecento carte. */
...
/* A che punto è la raccolta, dati i QR letti finora.
 *
 * Non è un errore passare doppioni o testi che non c'entrano niente: quelli
 * che non sono di OpenCard vengono ignorati, e *totale resta 0 finché non
 * arriva il primo pezzo buono. Chi arriva da un altro trasferimento fa
 * tornare OPENCARD_ERR_ALTRO_TRASF: è l'unico caso in cui la UI deve dire
 * qualcosa, perché vuol dire che i due telefoni si sono disallineati.
 */
 ...
 * se una tessera c'è già ci finisce due volte, e a toglierla è l'utente.
 ...
 * Tutte e due ritornano la lunghezza scritta, oppure -1 se out è troppo
 * piccolo o l'ingresso non è valido.
 */
